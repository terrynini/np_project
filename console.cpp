    ...
    if (getenv("QUERY_STRING") != "") {
        ...
        for (int i = 0; i < 5; i++) {
            if (GET_["h" + to_string(i)] != "" && GET_["p" + to_string(i)] != "" && GET_["f" + to_string(i)] != "") {
                ...
                if (ei != end) {
                    shared_ptr<shellSession> add = nullptr;
                    if (GET_["sh"] != "" && GET_["sp"] != "") {
                        ip::tcp::resolver::iterator sock = resolver.resolve(ip::tcp::resolver::query(GET_["sh"], GET_["sp"]));
                        if (sock != end) {
                            add = make_shared<shellSession>(move(*sock), io_service);
                            slist.push_back(add);
                        ...
                        add = make_shared<shellSession>(move(*ei), io_service);
                        slist.push_back(add);
                        add->server = GET_["h" + to_string(i)];
                        add->inputfile = GET_["f" + to_string(i)];
                        add->tag = "s" + to_string(slist.size() - 1);
